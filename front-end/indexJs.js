...
formInput.addEventListener('submit', function (d) { d.preventDefault(); });
...
function prepaireFetch(userinput) {
    ...
    myFetcher("https://api.tvmaze.com/search/shows?q=" + userinput.replace(" ", "+"));
...
function constructElement(dataMoviePic, dataMovieName, dataMovieRate) {
    ...
    fillMovieElement(dataMoviePicParam, dataMovieNameParam, dataMovieRateparam, moviePic, movieN, movieRate, watchListButtom);

}

function fillMovieElement(MoviePicParam, MovieNameParam, MovieRateparam, moviePicp, movieNp, movieRatep, watchListButtomp) {
